...
export function BadgeCard({
	...
	const { holderName, companyId, companyKey, companyLogo } = useMemo(
		() =>
			resolveBadgeData({
				props: { holderName: holderNameProp, companyId: companyIdProp, companyLogo: companyLogoProp, company: companyProp },
				user,
			}),
		...
		>
			...
			{companyLogo ? (
				<Box
					...
					sx={{ position: 'absolute', bottom: 10, right: 10, height: 24, objectFit: 'contain', pointerEvents: 'none' }}
					onError={(e) => { e.currentTarget.style.display = 'none'; }}
				/>
			) : (
				<Chip label={companyKey || 'AZIENDA'} size="small" sx={{ position: 'absolute', bottom: 10, right: 10, borderRadius: 1 }} />
			)}
			...
			>
				<Typography variant="subtitle1" sx={{ color: 'customBlue3.main' }}>{holderName || '—'}</Typography>
				<Typography variant="caption" sx={{ color: 'customBlue3.main' }}>{companyId || '—'}</Typography>
			</CardContent>
			{LogoGestionale && (
				<Box
					component="img"
					src={LogoGestionale}
					alt="app logo"
					sx={{ position: 'absolute', bottom: 8, left: 10, width: 30, height: 30, objectFit: 'contain', opacity: 0.95 }}
				/>
			)}
		</Card>
	);
