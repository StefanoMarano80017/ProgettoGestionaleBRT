...
export default function TaskItem({ task, employees, onManage }) {
  ...
  const assignedEmployees = employees.filter((emp) => task.assigned?.includes(emp.id));
  return (
    <Box sx={{ border: 1, borderColor: "divider", borderRadius: 1, overflow: "hidden", mb: 1 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", backgroundColor: "primary.light", color: "primary.contrastText", px: 2, py: 1.25 }}>
        <Typography variant="subtitle1" fontWeight="bold" sx={{ lineHeight: 1.2 }}>{task.title}</Typography>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", p: 1.5 }}>
        ...
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5, justifyContent: "flex-end", flexGrow: 1, ml: 2 }}>
          {assignedEmployees.map((emp) => (<Chip key={emp.id} label={emp.name} size="small" />))}
        </Box>
      </Box>
    </Box>
  );
