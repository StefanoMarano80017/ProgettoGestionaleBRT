...
// Migrated from Components/DipendenteHomePage/CommesseList.jsx
export default function CommesseList({ selectedDate }) {
  const [commesse, setCommesse] = React.useState([]);
  const [page, setPage] = React.useState(1);
  const pageSize = 5;

  const handleRemoveCommessa = (index) => { setDeleteIndex(index); setConfirmOpen(true); };
  ...
  return (
    <Card variant="outlined" sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <CardHeader
        title={ selectedDate ? selectedDate.toLocaleDateString('it-IT', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : 'Seleziona un giorno' }
      />
      <CardContent sx={{ flex: '1 1 auto', overflowY: 'auto', pt: 0, pb: 0, minHeight: 0, maxHeight: 'calc(100% - 120px)' }}>
        {commesse.length === 0 ? (
          <Box display="flex" justifyContent="center" alignItems="center" height="100px" p={2}>
            <Paper sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1 }} variant="outlined">
