...
export function useEmployeeFilter(employeesList = []) {
	...
	const filterFn = (items) => {
		...
		if (mode === 'some') {
			return items.filter((item) => value.some((empId) => item.assigned.includes(empId)));
		}
		if (mode === 'every') {
			return items.filter((item) => value.every((empId) => item.assigned.includes(empId)));
		}
		return items;
	};
	...
	return { value, set: setValue, filterFn, render, renderChip, clear, label: 'Dipendenti' };
}
