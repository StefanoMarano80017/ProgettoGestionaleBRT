...
export function useDayEntryEditing({
  ...
  const [form, setForm] = useState({ commessa: commesse[0] || '', ore: 1, descrizione: '' });
  ...
  const openEdit = (i) => {
    ...
    setMode('edit');
    setIdx(i);
    setForm({ commessa: r.commessa, ore: Number(r.ore || 1), descrizione: r.descrizione || '' });
    setError('');
    setOpen(true);
  };
  ...
  const save = () => {
    ...
    if (oreNum > maxOre) return setError(mode === 'add' ? `Puoi aggiungere al massimo ${maxOre}h` : `Puoi impostare al massimo ${maxOre}h per questa riga`);
    if (mode === 'add') {
      const newRecord = { dipendente: records[0]?.dipendente || 'Mario Rossi', commessa: form.commessa, ore: oreNum, descrizione: form.descrizione };
      onAddRecord(selectedDay, newRecord, false);
    } else if (mode === 'edit' && idx != null) {
      const next = [...records];
      next[idx] = { ...next[idx], commessa: form.commessa, ore: oreNum, descrizione: form.descrizione };
      onAddRecord(selectedDay, next, true);
    }
    setOpen(false);
  };
  return { dialogOpen: open, mode, idx, form, error, canAddMore, maxOre, setForm, openAdd, openEdit, close, save };
}
